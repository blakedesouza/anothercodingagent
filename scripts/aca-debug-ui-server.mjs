...
function summarizeConsultGroup(group, includePreviews) {
  ...
  for (const witnessName of witnessNames) {
    ...
    witnesses[witnessName] = {
      ...
      contextRequestDiagnostics: resultWitness?.context_request_diagnostics || [],
      contextAttemptDiagnostics: resultWitness?.context_attempt_diagnostics || [],
      finalizationDiagnostics: resultWitness?.finalization_diagnostics || [],
      contextSnippets: resultWitness?.context_snippets || [],
      safety: resultWitness?.safety || null,
      paths: {
        response: resultWitness?.response_path || null,
        rawRequest: resultWitness?.raw_request_path || null,
        triageInput: resultWitness?.triage_input_path || null,
      },
  ...
  return {
    ...
    sharedContext: {
      ...
      model: result?.shared_context?.model || null,
      error: result?.shared_context?.error || null,
      usage: result?.shared_context?.usage || null,
      contextRequests: result?.shared_context?.context_requests || [],
      contextRequestDiagnostics: result?.shared_context?.context_request_diagnostics || [],
      contextSnippets: result?.shared_context?.context_snippets || [],
      provenanceSummary: result?.shared_context?.provenance_summary || [],
