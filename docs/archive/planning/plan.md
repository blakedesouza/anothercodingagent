...
- C11.1 (stress-test battery, baseline): **COMPLETE** — 5 scenarios × 6 models run; docs/archive/audits/c11/failure-catalog.md written. Key findings: (1) DeepSeek `llm.malformed` after 62KB tool results (S4, P1); (2) Qwen `reasoning_content` leaks into invoke results (P2); (3) MiniMax result-narration confirmed (P3). All stall-prone models passed S1; consult pipeline clean (S3).
...
**Kimi/qwen model hints: COMPLETE (2026-04-10).** Both hints added to `src/prompts/model-hints.ts`. 4-test retest passed (1-retry 4/4, 3-retry 4/4, A 4/4, B 4/4). 2632 tests still passing.
...
**C6 RP workflow improvements — COMPLETE (2026-04-10).** (1) GLM-5 set as default model for `rp-research` (no `--model` flag required). (2) Spoiler validator regex tightened — `mystery` removed from alternation, parenthetical now requires uppercase-initial token to avoid false-positives on genre-description text. (3) Parallel generation: `createLimiter` semaphore added, three generation phases (world/locations/characters) now run up to 4 concurrent invoke tasks (`--concurrency` flag, clamped 1–8). (4) Session salvage race fixed: `randomUUID` tag generated per `runRpInvokeTask` attempt, propagated via `ACA_SESSION_TAG` env var, persisted in invoke session `configSnapshot.sessionTag`, matched in `findFreshSessionDir`. Live validated: all 18 Quints files generated in one run; session tags confirmed via manifest spot-check.

**GLM-5 narration fix + shell ergonomics (2026-04-10): COMPLETE.** Root cause: GLM-5 (thinking model) emits bare intent text ("I'll now fetch...") in the visible response after making calls in an earlier step, causing `assistant_final` yield with no output written. Three root causes found via Opus analysis: (1) no GLM-5 model hints; (2) `validateProfileCompletion` only fired when cumulative tool calls = 0, missing the narrate-after-work pattern; (3) `canRepairMissingOutputs` used post-repair tool count, so if the profile-repair also narrated the output repair didn't fire. Fixes: (1) `zai-org/glm` entry in `src/prompts/model-hints.ts` — two anti-narration directives; (2) `validateProfileCompletion` extended with optional `lastStepAcceptedToolCalls` + `missingRequiredPaths` — fires when last step had 0 calls and outputs are still missing; (3) `cli-main.ts` tracks `originalTotalToolCalls` separately so `canRepairMissingOutputs` fires even when the profile-repair also narrated. 2 regression tests. Live validated: 17/17 Quints files generated. Shell: `rp()` function + `aca` alias added to `~/.bashrc`. Config: `model.default = zai-org/glm-5` set in both `~/.aca/config.json` and `.aca/config.json`. REPL/one-shot model resolution fixed to fall back to `config.model?.default` (was hard-erroring when `--model` flag absent). Changelog: `docs/changelog.md`.

**C11 Gauntlet: COMPLETE (2026-04-10, 10/10 PASS after fixes).** 10-test end-to-end regression run. 01–08 PASS, 10 PASS. Test 09 PARTIAL FAIL → root cause found and fixed (commit `2a69263`): all 3 `buildContextRequestPrompt`/`buildContinuationPrompt`/`buildSharedContextRequestPrompt` sites had literal `"src/path/to/file.ts"` + `"short concrete reason"` in worked-example JSON blocks. qwen copied them verbatim. Fix: replaced both with angle-bracket tokens (`<real/repo/relative/path.ts>`, `<your specific reason>`). Follow-up Opus impact audit (commit `f467b30`) found: (1) the "fails JSON.parse" defense was wrong — angle brackets are valid JSON string chars; (2) `normalizeContextRequests` now hard-rejects paths containing `<`/`>`; (3) tree example at line 188 also converted to angle-bracket placeholders; (4) no pseudo-tool-call false positives, no other contaminated sites, triage/finalization prompts clean. Live-validated: qwen test-09 re-run PASS (requested `src/consult/context-request.ts:340-380` with real reason). 2661 tests still passing.
...
- Live test results (deepseek, 5 questions, 3-round max):
  - Test 1 (maxOutputTokens config): ok | 3 rounds | 3 tree requests | 0 ENOENT ✓
  - Test 2 (consult pipeline files): ok | 3 rounds | 4 tree requests | 0 ENOENT ✓
  - Test 3 (finalization prompt guardrails): ERROR — 2 ENOENT (guessed src/consult/consult.ts, src/prompts/consult.ts despite trees); finalization produced non-report output. Pre-existing model path-hallucination issue, not a C11.7 regression.
...
**ACA-native safety boundary (2026-04-06):** Local Claude/Codex consult skills may be used as test harnesses and convenience launchers, but they are not the product boundary. Safety-critical witness/delegation behavior must live in tracked ACA code: invoke caps, per-tool caps, tool-result/input-token guards, denied-tool enforcement, safety telemetry, deterministic evidence packing, context-request snippet fulfillment, no-tools witness finalization, triage profiles, and result artifacts. Current productization path: `aca consult` owns the context-request consult workflow; local skills should remain thin adapters around that ACA-native surface.
**RP research profile direction (2026-04-06, revised 2026-04-08):** Kimi K2.5 was the first default writer/researcher for RP lore docs; GLM-5 is now the stronger local high-trust candidate when available through NanoGPT. The `rp-researcher` profile is a bounded Markdown writer for caller-selected RP project folders: web_search/fetch_url/fetch_mediawiki_page/fetch_mediawiki_category for source discovery, read_file/find_paths/search_text for existing house style, make_directory/write_file for exact orchestrator-assigned output paths. It intentionally excludes exec_command, edit/delete/move, browser tools, and delegation. Review should reuse existing witness profiles; triage should reuse the aggregation profile. After the first Odd Taxi dry run, `aca invoke` now accepts `context.profile` so this profile can be applied by contract instead of pasted into task text. After Fandom security-verification pages, ACA added bounded MediaWiki/Fandom `api.php` tools (`fetch_mediawiki_page`, `fetch_mediawiki_category`) so RP research can avoid Playwright/browser escalation for standard wiki pages. Broad RP compendiums should not bottleneck through one compact global research brief: run discovery to enumerate characters/topics and exact output paths, then run one deep GLM-5 research/write invocation per character or world file with `ACA_NETWORK_MODE=open` for local high-trust research, exact `required_output_paths`, and `fail_on_rejected_tool_calls`. Middle-ground depth ceilings are the default: main character files up to 16-20 KB, side characters 8-12 KB, minor/supporting characters 4-8 KB, with no padding when canon material is thin. Relationships stay to important dynamics with 1-2 sentences each, and Japanese script is avoided unless needed to disambiguate an ability/skill/magic name.

**Consult triage default (2026-04-08):** ACA-native `aca consult` now uses `zai-org/glm-5` for no-tools triage aggregation. GLM-5.1 is no longer exposed on the live NanoGPT subscription catalog, so shared-context and triage were moved back to GLM-5; keep triage no-tools by default.
**C1 verification + M7A.5 wiring (2026-04-09, COMPLETE):** Independent Opus verification of Codex's C1 closure found `src/review/*` (4 files, ~1000 lines, 128 tests, M7A.5.1-M7A.5.4) has zero runtime callers — tree-shaken from dist/, never wired into consult pipeline despite being marked COMPLETE. Fixed: `src/review/markdown-adapter.ts` (new) extracts structured `WitnessReview` from freeform witness Markdown using heuristic heading/bullet parsing; wired into `src/cli/consult.ts` aggregation block that writes `.md`+`.json` artifacts to `<temp>/`. `ConsultResult.structured_review` field added. 141 review tests pass. Live-tested across 4 witnesses: `status:ok` confirmed. Commits: `a42ae88`, `200af40`, `c36b731`.
**NanoGPT driver reasoning_content fix (2026-04-09, COMPLETE):** Qwen3 thinking models emit chain-of-thought in `delta.reasoning_content` (not `delta.content`). ACA only read `delta.content`, so thinking-only responses produced `assistantParts.length === 0` → `llm.malformed` abort. Root cause confirmed via C7 probe session forensics (`ses_01KNSWTTVP`: 20 output tokens, outputSeqs=[]). Fix: capture `delta.reasoning_content` as `text_delta` in nanogpt-driver. Verified against Alibaba Cloud Model Studio streaming docs. Commit: `c53a77d`.
