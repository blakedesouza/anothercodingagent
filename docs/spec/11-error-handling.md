...
- **Block 8 (Permissions):** Sandbox denials produce `tool.permission_denied`. Approval-required is a yield condition, not an error — it uses the existing `approval_required` turn outcome, not an error code. The `--no-confirm` flag overrides ordinary `confirm` decisions but not `confirm_always`, high-risk shell commands, or `deny` (unchanged)
