...
**RP research profile direction (2026-04-06):** Kimi K2.5 was the first default writer/researcher for RP lore docs; GLM-5 is now a strong candidate when available through NanoGPT. The `rp-researcher` profile is a bounded Markdown writer for caller-selected RP project folders: web_search/fetch_url/fetch_mediawiki_page/fetch_mediawiki_category for source discovery, read_file/find_paths/search_text for existing house style, make_directory/write_file for exact orchestrator-assigned output paths. It intentionally excludes exec_command, edit/delete/move, browser tools, and delegation. Review should reuse existing witness profiles; triage should reuse the aggregation profile. After the first Odd Taxi dry run, `aca invoke` now accepts `context.profile` so this profile can be applied by contract instead of pasted into task text. After Fandom security-verification pages, ACA added bounded MediaWiki/Fandom `api.php` tools (`fetch_mediawiki_page`, `fetch_mediawiki_category`) so RP research can avoid Playwright/browser escalation for standard wiki pages. Broad compendiums should use reusable dynamic passes: discovery for category/page enumeration; orchestrator planning to group discovered pages into research batches; bounded GLM/Kimi research passes for exact batches; then a write-only pass with exact `required_output_paths` and `fail_on_rejected_tool_calls`.

**Consult triage default (2026-04-07):** ACA-native `aca consult` now uses `zai-org/glm-5` for no-tools triage aggregation instead of DeepSeek. GLM-5 showed stronger bounded behavior in RP research and passed a synthetic no-tools triage smoke with one-step aggregation and zero tool calls; keep triage no-tools by default.
