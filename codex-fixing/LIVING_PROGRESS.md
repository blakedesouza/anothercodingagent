...
- `C6` is a new productizing follow-on track rather than a bug-only residual pass: it turns the RP research workflow into a first-class "research `<series>` for RP" import path, defaulting to `/home/blake/projects/rpproject/<series-slug>/` on this machine and calibrating the output format on the anime version of *The Quintessential Quintuplets*
- `C6` now has a formal authoring contract in `RP_AUTHORING_CONTRACT.md`, and the current Quints calibration pack was intentionally archived at `/home/blake/projects/rpproject/_archive/the-quintessential-quintuplets-c6-pre-regen-2026-04-09` before deleting the live `/home/blake/projects/rpproject/the-quintessential-quintuplets` folder so future regeneration can start clean
...
- Fresh live `C7` evidence now exists:
  ...
  - built `invoke` with `profile: "rp-researcher"` created `/home/blake/projects/.aca-c7-rp-probe/royal-biblia-academy.md` through accepted `read_file` + `write_file`, then hit a retryable post-write `llm.malformed` abort; the write path itself is live, but the model can still terminate badly after satisfying the file contract
  - a built no-tools DeepSeek consult against `RP_AUTHORING_CONTRACT.md` stayed in clean `needs_context` / snippet fulfillment mode and did not drift into pseudo-tool markup
...
- hardened `/home/blake/.claude/skills/delegate/scripts/run_delegate.py` so shared-`HOME` runs now match the created ACA session back to the task text and emit exact `session_id` / `session_dir`, then aligned the `.claude` and `.codex` delegate/orchestrate skill docs to that artifact shape
